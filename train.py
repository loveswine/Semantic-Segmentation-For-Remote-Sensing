...
def parse_args():
    ...
    # dataset
    parser.add_argument('--dataset-name', type=str, default='water')
    parser.add_argument('--train-data-root', type=str, default=r'./dataset/train/512')
    parser.add_argument('--valpercent', default=0.1)
    ...
    # model
    parser.add_argument('--model', type=str, default='deeplabv3_version_2', help='model name')
    ...
    parser.add_argument('--class-loss-weight', type=list,
                        default=[0.19803182310947887, 0.2423774991181078, 0.8802022721998504, 0.2854556475227468, 1.0],
                        help='for unbalanced categories, use ./tools/class_weight.py to calculate')

    parser.add_argument('--momentum', type=float, default=0.9, metavar='M', help='momentum (default:0.9)')
    parser.add_argument('--weight-decay', type=float, default=0.0001, metavar='M', help='weight-decay (default:1e-4)')
    # optimizer
    parser.add_argument('--optimizer-name', type=str, default='Adadelta')#'Adadelta')
    # learning_rate
    ...
    parser.add_argument('--best-kappa', type=float, default=0.8)
...
class Trainer(object):
    def __init__(self, args):
        ...
        print(args.train_data_root)
        datasets = RSDataset(root=args.train_data_root, mode='train', sync_transforms=sync_transform)
        
        n_val = int(len(datasets) * args.valpercent)
        n_train = len(datasets) - n_val
        self.train_dataset, self.val_dataset = random_split(datasets, [n_train, n_val],
        ...
        self.train_loader = DataLoader(dataset=self.train_dataset,
                                       ...
                                       shuffle=True,
                                       drop_last=True)
        print('class names {}'.format(self.train_dataset.dataset.class_names))
        print('Train samples {}'.format(len(self.train_dataset)))
        if not args.no_val:
            self.val_loader = DataLoader(dataset=self.val_dataset,
                                         ...
                                         shuffle=False,
                                         drop_last=True)
        self.num_classes = len(self.train_dataset.dataset.class_names)
        print('Val samples {}'.format(len(self.val_dataset)))
        ...
        self.lr_adjust=torch.optim.lr_scheduler.ExponentialLR(self.optimizer,gamma=0.98)
    ...
    def training(self, epoch):
        ...
        for index, data in enumerate(tbar):
            ...
            tbar.set_description('epoch {}, training loss {}, with learning rate {}.'.format(
                epoch, train_loss.avg, self.lr_adjust.get_last_lr()[0]
            ))
        ...
        for i in range(self.num_classes):
            table.add_row([i, self.train_dataset.dataset.class_names[i], train_acc_per_class[i], train_IoU[i]])
        print(table)
    ...
    def validating(self, epoch):
        ...
        for index, data in enumerate(tbar):
            ...
            if val_visual:
                ...
                val_visual = torchvision.utils.make_grid(tensor=val_visual,
                                                         ...
                                                         scale_each=False,
                                                         pad_value=0)
                writer.add_image(tag='img&GTs&pres', img_tensor=val_visual, global_step=None, walltime=None)
            conf_mat += metric.confusion_matrix(pred=preds.flatten(),
                                                label=masks.flatten(),
                                                num_classes=self.num_classes)
        ...
        writer.add_scalars(main_tag='val_single_acc',
                           tag_scalar_dict={self.train_dataset.dataset.class_names[i]: val_acc_per_class[i] for i in
                                            range(len(self.train_dataset.dataset.class_names))},
                           global_step=epoch, walltime=None)
        writer.add_scalars(main_tag='val_single_iou',
                           tag_scalar_dict={self.train_dataset.dataset.class_names[i]: val_IoU[i] for i in
                                            range(len(self.train_dataset.dataset.class_names))},
                           global_step=epoch, walltime=None)
        ...
        table = PrettyTable(["序号", "名称", "acc", "IoU"])
        for i in range(self.num_classes):
            table.add_row([i, self.train_dataset.dataset.class_names[i], val_acc_per_class[i], val_IoU[i]])
        print(table)
